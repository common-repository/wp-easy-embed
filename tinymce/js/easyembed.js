var wpEasyEmbed = (function(){
	var main = window.dialogArguments || opener || parent || top;
	var jQuery = main.jQuery;
	var URLValue = null;
	var tabs = ['video', 'image', 'link', 'rich'];
	var root = null;
	var loadingHTML = '<div align="center"><img src="' + main.wpEasyEmbedConfig().loadingIconURL + '" /></div>';
	...
	function onURLChange (){
		...
		timerURLChange = setTimeout(function(){
			wpEasyEmbed.checkURLProviders(URLValue);
		}, 300);
	}
	/**
	...
	return {
		...
		, submit : function(form){
			...
			jQuery(values).each(function(){
				...
				if(this.name == "url" || this.name == "provider" || this.name == "type" || this.name == '' || this.value == ''){
					return;
				}
		...
		, checkURLProviders : function(url) {
			var providers = main.wpEasyEmbedConfig().providers;
		...
		, fetchAdditionalFields : function(){
			jQuery.post(main.ajaxurl, {
				action : 'wp_easy_embed_additional_fields'
...
window.onload = wpEasyEmbed.init;
wpEasyEmbed.preInit();